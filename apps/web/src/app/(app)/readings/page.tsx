...
function buildReadingsPageHref(params: {
  page: number;
  siteId?: string;
  deviceId?: string;
  sensorType?: string;
  ...
  return `/readings?${search.toString()}`;
}
...
export default async function ReadingsPage({
  ...
  const totalPages = Math.max(1, Math.ceil(totalReadings / PAGE_SIZE));
  ...
  const startItem = totalReadings === 0 ? 0 : skip + 1;
  const endItem = Math.min(skip + PAGE_SIZE, totalReadings);

  const previousPageHref = buildReadingsPageHref({
  ...
  const nextPageHref = buildReadingsPageHref({
        ...
        {readings.length === 0 ? (
          ...
          <>
            <div className="border-b border-slate-200 px-4 py-3 text-sm text-slate-500 dark:border-slate-800 dark:text-slate-400 sm:px-5">
              Showing {startItem} ~ {endItem} of {totalReadings} readings
            </div>
